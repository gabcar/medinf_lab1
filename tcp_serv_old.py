...
def splitData(packages):
    ...
    return signal, mean, time

def plotData(signal, mean, time):
    plt.ion()
    fig = plt.figure()
    window_size = 100
...
total_packages = getConnection()
signal, mean, time = splitData(total_packages)
plotData(signal, mean, time)