...
def ecgStream():
    ...
    while True:
        ...
        if change_time != prev_time: #check if prev time == time of last change
            ...
            bpm = [int(random.randint(50,120)) for i in range(len(bpm))]
            bpm_mean = int(round(sum(map(int,bpm)))/len(bpm))
            s1.write(dict(x=time, y=signal),dict(title='bpm: '+str(bpm_mean)))
